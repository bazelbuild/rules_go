...
def _go_import_path(ctx, label):
  """Returns the expected Go importpath of the given label

  Args:
    ctx: The skylark Context.
    label: A Label of a go_library, go_binary or go_test .
  """
  prefix = _go_prefix(ctx)
  path = prefix + label.package + "/" + label.name
  if label.name == _DEFAULT_LIB:
    path = prefix + label.package
