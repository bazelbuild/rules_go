...
func run(args []string) error {
	...
	packageFile, importMap, err := parseImportCfg(*importcfg)
	if err != nil {
...
// parseImportCfg parses the importcfg file generated by the compile program
// and produces the packageFiles and importMap maps used by the vet and checker
// importers.
func parseImportCfg(importcfg string) (packageFiles map[string]string, importMap map[string]string, err error) {
	importcfgTxt, err := ioutil.ReadFile(importcfg)
	if err != nil {
		return nil, nil, err
	}
	packageFiles, importMap = make(map[string]string), make(map[string]string)
	for _, line := range strings.Split(string(importcfgTxt), "\n") {
		if line == "" {
			continue
		}
		var isPackageFile bool
		switch {
		case strings.HasPrefix(line, "packagefile "):
			line = strings.TrimPrefix(line, "packagefile ")
			isPackageFile = true
		case strings.HasPrefix(line, "importmap "):
			line = strings.TrimPrefix(line, "importmap ")
			isPackageFile = false
		default:
			return nil, nil, fmt.Errorf("invalid importcfg line %q", line)
		}

		i := strings.Index(line, "=")
		if i < 0 {
			return nil, nil, fmt.Errorf("invalid key-value entry %q: no = separator", line)
		}
		k, v := line[:i], line[i+1:]
		if isPackageFile {
			packageFiles[k] = v
