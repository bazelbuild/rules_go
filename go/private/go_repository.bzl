...
def _go_repository_impl(ctx):
  if ctx.attr.urls:
    ...
    _fetch_repo = "@io_bazel_rules_go_repository_tools//:bin/fetch_repo{}".format(check_extension(ctx))
    result = env_execute(
        ctx,
        [
  ...
  if generate:
    ...
    _gazelle = "@io_bazel_rules_go_repository_tools//:bin/gazelle{}".format(check_extension(ctx))
